...
| Repository-controlled execution | Policy forbids tests/builds/scripts; no shell interpolation; Git is fixed-argument and hardened; hardened mode provides no-exec temporary state, read-only image/repository mounts, non-root execution, dropped capabilities, and no privilege escalation | The image retains the minimal base utilities needed by Python/Codex; policy remains necessary and a compromised dependency may execute available binaries |
| Path traversal/symlink escape | Canonical allowed roots, `relative_to` containment, explicit `..` rejection, no-follow bounded descriptor reads, canonical evidence recheck | Concurrent directory namespace replacement cannot be fully excluded without OS mount/namespace isolation |
...
| Malformed/malicious model output | One turn, output schema, strict Pydantic envelope/result, semantic invariants, evidence and total-size bounds | Novel secret encodings or misleading concise findings may evade heuristic redaction |
...
path enter application memory. The child gets a non-secret request, explicit credentials via
